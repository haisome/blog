...
from .mideaPunch import *
from .saveemployee import *
import datetime
import ssl
...
class MideaAuthenticateView(View):
    def post(self, request):
        user = UserProfile.objects.get(pk=request.user.pk)
        user.midea_user = request.POST.get('mideauser', '')  # 获取用户名
        user.midea_password = request.POST.get('password', '')  # 获取密码
        token=getToken(user.midea_user,user.midea_password)  #获取ssotoken
        MideaUser = MideaUserProfile.objects.get(jid= user.midea_user+'@im.midea.com')
        user.midea_cardnum = MideaUser.number

        if token[0].get('1') is not None and token[0].get('5') is None:
            user.midea_ssotoken = token[1]
            user.midea_authenticated=True
            Message.objects.create(body="用户%s于%s认证Midea用户%s成功" % (request.user.email, timezone.now(),user.midea_user))
            user.save()
        else:
            user.midea_authenticated=False
            Message.objects.create(body="用户%s于%s认证Midea用户%s失败" % (request.user.email, timezone.now(),user.midea_user))
            user.save()
    ...
    def get(self, request):
        ...
        if getType == 'updateMidea':
            ...
            if result ==-1:
                ...
                for item in result:
                    #    print (item)
                    EffCard.objects.get_or_create(oid = item['oid'],cardName = item['cardName'])
            ...
            result = doGet('getAttendCardrecord',getAttendCardrecord,user.midea_cardnum,user.midea_ssotoken,[date,''])
            #result= '''<?xml version='1.0' encoding='utf-8'?><soapenv:Envelope xmlns:soapenv="http://www.w3.org/2003/05/soap-envelope"><soapenv:Body><ns:getAttendCardrecordResponse xmlns:ns="http://services.webservices.com"><ns:return>[{"c_devname":"美的总部公交站","c_unitname":"美的控股_关联单位_IT外包资源管理_系统运维_应用维护模块","c_kqdatetime":"2017-06-21 14:27:46"},{"c_devname":"集团总部","c_unitname":"美的控股_关联单位_IT外包资源管理_系统运维_应用维护模块","c_kqdatetime":"2017-06-21 08:23:09"}]</ns:return></ns:getAttendCardrecordResponse></soapenv:Body></soapenv:Envelope>'''
