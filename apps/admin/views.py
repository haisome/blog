...
'''
#上传到七牛后保存的文件名
#key = 'WechatIMG89.jpeg';
#生成上传 Token，可以指定过期时间等
#token = q.upload_token(bucket_name, key, 3600)
#要上传文件的本地路径
#localfile = './Hydrangeas.jpg'
...
class DashboardView(LoginRequiredMixin, View):
    def get(self, request):
        effCards = EffCard.objects.all()
        today=datetime.datetime.now().strftime('%Y-%m-%d')
        #return render(request, 'dashboard.html', {'effCards': effCards,'today':today,})
        response = render(request, 'dashboard.html', {'effCards': effCards,'today':today,})
...
class UploadView(FormView):
    def post(self, request, *args, **kwargs):
        ...
        if files:
            ...
            localfile = settings.MEDIA_ROOT + str(img.image)
            key = "".join(str(img.image).split('/')[2:])
            token = q.upload_token(bucket_name, key, 3600)
            print ('token=',token,'key=',key,'localfile=',localfile)
            ret, info = put_file(token, key, localfile)
            print(info)
            assert ret['key'] == key
            assert ret['hash'] == etag(localfile)
            result = {'success': 1, 'message': 'OK', 'url': settings.BUCKET_URL + key }
            #result = {'success': 1, 'message': 'OK', 'url': settings.MEDIA_URL + str(img.image)}
        else:
